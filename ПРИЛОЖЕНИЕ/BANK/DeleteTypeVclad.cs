...
namespace BANK
{
    public partial class DeleteTypeVclad : Form
    {
        DataTable table = new DataTable();
        SqlDataAdapter sqlData = new SqlDataAdapter();
        SqlCommand sqlCommand = new SqlCommand();

        public DeleteTypeVclad()
        {
            InitializeComponent();
        }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                sqlCommand.CommandText = @"DELETE [ВИДЫ_ВКЛАДОВ] WHERE [КОД_УСЛУГИ] = @NUM";
        ...
        private void DeleteTypeVclad_Load(object sender, EventArgs e)
        {
            sqlCommand.CommandText = "SELECT КОД_УСЛУГИ FROM [ВИДЫ_ВКЛАДОВ]";
            sqlCommand.Connection = MyConnection.Connection;
