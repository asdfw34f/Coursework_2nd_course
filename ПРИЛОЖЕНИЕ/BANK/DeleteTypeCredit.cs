...
namespace BANK
{
    public partial class DeleteTypeCredit : Form
    {
        DataTable table = new DataTable();
        SqlDataAdapter sqlData = new SqlDataAdapter();
        SqlCommand sqlCommand = new SqlCommand();

        public DeleteTypeCredit()
        {
            InitializeComponent();
        }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                sqlCommand.CommandText = @"DELETE [ВИДЫ_КРЕДИТОВ] WHERE [КОД_УСЛУГИ] = @NUM";
        ...
        private void DeleteTypeCredit_Load(object sender, EventArgs e)
        {
            sqlCommand.CommandText = "SELECT КОД_УСЛУГИ FROM [ВИДЫ_КРЕДИТОВ]";
            sqlCommand.Connection = MyConnection.Connection;
