...
namespace BANK
{
    public partial class AddWorkers : Form
    {
        ...
        SqlCommand command = new SqlCommand(@"Insert into [СОТРУДНИКИ]([НОМЕР_СОТРУДНИКА], [ИМЯ], [ОТЧЕСТВО], [ФАМИЛИЯ], [ДАТА_РОЖДЕНИЯ], [ПОЛ], [ТЕЛЕФОН], [EMAIL], [АДРЕС_РЕГИСТРАЦИИ], [ФАКТИЧЕСКИЙ_АДРЕС], [СЕРИЯ_НОМЕР]) VALUES(@NUMBER, @NAME, @OTCHE, @FAMILY, @DATEBIRTHDAY, @POL, @PHONE, @EMAIL, @ADRESSREG, @ADRESSFACT, @PASS)", MyConnection.GetConnection());
        public AddWorkers()
        {
            InitializeComponent();
        }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            //@NUMBER, @NAME, @OTCHE, @FAMILY, @DATEBIRTHDAY, @POL, @PHONE, @EMAIL, @ADRESSREG, @ADRESSFACT, @PASS
            try
            {
        ...
        private void AddWorkers_Load(object sender, EventArgs e)
        {
